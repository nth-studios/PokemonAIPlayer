...
class PolicyNetwork(tf.keras.Model):
    def __init__(self):
        ...
        self.dense4 = tf.keras.layers.Dense(len(outputs))  # logits output
...
outputs = ["a", "b", "left", "right", "up", "down"]
...
def train(policy, optimizer, episodes=1000):
    for episode in range(episodes):
        ...
        while count < time_train:
            ...
            action = np.random.choice(len(outputs), p=action_prob)
            action = np.random.choice([action, np.random.choice(len(outputs))], p=[1-rand_chance, rand_chance])
